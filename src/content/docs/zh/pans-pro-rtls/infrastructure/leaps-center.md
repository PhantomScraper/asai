...
sourceUrl: "https://docs.leapslabs.com/zh-cn/pans-pro-rtls/infrastructure/leaps-center/"
order: 98
---
...
<p><a class="reference internal" href="#pans-pro-center"><span class="std std-ref">LEAPS Center</span></a> 是一个web应用程序，它为整个网络提供设备管理, 网络管理以及位置和遥测数据的可视化。</p>
<div class="section" id="key-features">
<h2>主要特点</h2>
<ul class="simple">
<li><p>二维和三维网格可提供实时位置更新和网络中设备的可视化。</p></li>
<li><p>其他有用的功能包括用户管理, 区域管理, 区域历史, 平面图管理, 位置历史和位置热图。</p></li>
<li><p>The <a class="reference internal" href="#pans-pro-center"><span class="std std-ref">LEAPS Center</span></a> 通过 <a class="reference external" href="https://mosquitto.org/">MQTT Broker</a> 与 <a class="reference internal" href="/docs/zh/pans-pro-rtls/infrastructure/leaps-server#pans-pro-server"><span class="std std-ref">LEAPS Server</span></a> 互联. 它作为一项服务在 Linux 和 Windows 上运行</p></li>
</ul>
</div>
...
<blockquote>
<div><ul class="simple">
<li><p>创建包含 <a class="reference internal" href="#pans-configuration-files"><span class="std std-ref">配置文件</span></a> 的 leaps_center 目录</p>
<ul>
...
<li><p>或者下载 LEAPS Center配置包 (<a class="reference external" href="https://drive.google.com/file/d/1T-TECozBs9O87w4sCUHM1bXtLkd6jvHM/view?usp=drive_link">LEAPS_CENTER_DOCKER.zip</a>)。</p></li>
</ul>
</div></blockquote>
...
<blockquote>
<div><ul class="simple">
<li><p>导航至从 LEAPS Center配置包中提取的目录。</p></li>
</ul>
...
<li><p>安装 LEAPS Center Docker 软件包并运行：</p></li>
</ol>
<blockquote>
...
<li><p>打开网络浏览器，输入以下 <a class="reference external" href="http://localhost:80">http://localhost:80</a>；LEAPS Center应用程序将在网络浏览器中加载。</p></li>
</ol>
<blockquote>
...
<p>至此，您已在电脑上成功安装并启动了 LEAPS Center。</p>
</div>
</div>
<div class="section" id="configuration-files">
<span id="pans-configuration-files"></span><h2>配置文件</h2>
<ul>
...
<ul class="simple">
<li><p>启动 LEAPS Center，运行 docker start leaps_center</p></li>
<li><p>停止 LEAPS Center，运行 docker stop leaps_center</p></li>
<li><p>重启 LEAPS Center，运行 docker restart leaps_center</p></li>
<li><p>移除 LEAPS Center，运行 <code class="docutils literal notranslate"><span class="pre">docker</span> <span class="pre">rm</span> <span class="pre">--force</span> <span class="pre">leaps_center</span></code></p></li>
</ul>
</div>
...
<li><p>由于 LEAPS Center会在用户登录后以及未授权时执行重定向，因此有必要在用户客户端正确设置 “跟随重定向” 参数。 登录后的响应代码为 302，但用户需要手动处理，因为如果用户未获授权，状态代码也是 302。 例如，用户可以向 REST 服务提出任何请求，如果响应状态代码不是 200，用户就不会获得授权。</p></li>
</ol>
</div>
</li>
</ol>
...
<h2>故障排除</h2>
<ul class="simple">
<li><p>使用以下命令 <code class="docutils literal notranslate"><span class="pre">docker</span> <span class="pre">restart</span> <span class="pre">leaps_center</span></code> 重启 LEAPS Center。</p></li>
<li><p>在LEAPS Center运行时检查日志，打开 docker 桌面并选择 leaps_center 容器。</p></li>
